...
PointPillars requires generated samples that store `faulty_lidar_points` and
aligned View-of-Delft radar caches that store five-column `radar_points` as
`[x,y,z,power,doppler]`. Obsolete artifacts are rejected rather than silently
reconstructing points from BEV rasters.
