...
import models.reconstruction_head.coarse_reconstruction.sst_backbone as sst_module
...
class SSTReconstructionTests(unittest.TestCase):
    ...
    def test_reused_region_layout_matches_recomputed_layout(self):
        config = SSTConfig(
            token_dim=8,
            num_blocks=1,
            num_heads=2,
        ...
        attention = SparseRegionalAttention(
            config, shift_size_cells=2
        ).eval()
        coordinates = torch.tensor(
            [[0, 0, 0], [0, 2, 3], [0, 5, 5], [1, 7, 7]]
        )
        features = torch.randn(4, 8)
        ...
        with torch.no_grad():
            recomputed, recomputed_counts, _ = attention(
                features, coordinates, (8, 8)
            )
            reused, reused_counts, _ = attention(
                features,
                coordinates,
                (8, 8),
                layout=layout,
            )
        self.assertTrue(torch.equal(recomputed_counts, reused_counts))
        self.assertTrue(torch.equal(recomputed, reused))
