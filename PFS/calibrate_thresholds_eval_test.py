...
def collect_probabilities(model, loader, device, metric_grid_size):
    outputs = []
    targets = []
    model.eval()
    with torch.no_grad():
...
def evaluate_dataset(model, loader, device, threshold, metric_grid_size, boundary_chamfer):
...
def main():
    ...
    val_paths = list_npz(val_root)
    test_paths = list_npz(test_root)
    ...
    val_outputs, val_targets = collect_probabilities(model, val_loader, device, args.grid_size)
    val_sweep_rows = threshold_sweep(val_outputs, val_targets, args.thresholds, metric_grid_size=None)
